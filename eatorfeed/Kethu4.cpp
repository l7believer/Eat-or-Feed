...
void Kethu4::loadSpriteSheet(const std::string& leftPath, const std::string& rightPath, SDL_Renderer* renderer) {
...
void Kethu4::updatePosition() {
    moveToTarget();
}

void Kethu4::setRandomTarget() {
...
void Kethu4::render(SDL_Renderer* renderer, SDL_Rect camera) {
    SDL_Texture* currentSprite = facingLeft ? spriteLeft : spriteRight;
    const int frameWidth = 2716 / 15;
    const int frameHeight = 544 / 3;
    SDL_Rect srcRect;
...
float Kethu4::getX() const { return x; }
float Kethu4::getY() const { return y; }

SDL_Rect Kethu4::getCollisionBox() const {
    const int frameWidth = 2716 / 15;
    const int frameHeight = 544 / 3;
