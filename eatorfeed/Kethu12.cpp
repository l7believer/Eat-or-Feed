...
void Kethu12::loadSpriteSheet(const std::string& leftPath, const std::string& rightPath, SDL_Renderer* renderer) {
...
void Kethu12::updatePosition() {
    moveToTarget();
}

void Kethu12::setRandomTarget() {
...
void Kethu12::render(SDL_Renderer* renderer, SDL_Rect camera) {
    SDL_Texture* currentSprite = facingLeft ? spriteLeft : spriteRight;
    const int frameWidth = 3617 / 15;
    const int frameHeight = 364 / 3;
    SDL_Rect srcRect;
...
float Kethu12::getX() const { return x; }
float Kethu12::getY() const { return y; }

SDL_Rect Kethu12::getCollisionBox() const {
    const int frameWidth = 3617 / 15;
    const int frameHeight = 364 / 3;
