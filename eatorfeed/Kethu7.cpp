...
void Kethu7::loadSpriteSheet(const std::string& leftPath, const std::string& rightPath, SDL_Renderer* renderer) {
    SDL_Surface* leftSurface = IMG_Load(leftPath.c_str());
    SDL_Surface* rightSurface = IMG_Load(rightPath.c_str());

    SDL_SetColorKey(leftSurface, SDL_TRUE, SDL_MapRGB(leftSurface->format, 108, 92, 187));
    SDL_SetColorKey(rightSurface, SDL_TRUE, SDL_MapRGB(rightSurface->format, 108, 92, 187));
...
void Kethu7::updatePosition() {
    moveToTarget();
}

void Kethu7::setRandomTarget() {
...
void Kethu7::render(SDL_Renderer* renderer, SDL_Rect camera) {
    SDL_Texture* currentSprite = facingLeft ? spriteLeft : spriteRight;
    const int frameWidth = 991 / 15;
    const int frameHeight = 99 / 2;
    SDL_Rect srcRect;
...
float Kethu7::getX() const { return x; }
float Kethu7::getY() const { return y; }

SDL_Rect Kethu7::getCollisionBox() const {
    const int frameWidth = 991 / 15;
    const int frameHeight = 99 / 2;
