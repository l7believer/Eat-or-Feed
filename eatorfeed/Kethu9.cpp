...
void Kethu9::loadSpriteSheet(const std::string& leftPath, const std::string& rightPath, SDL_Renderer* renderer) {
...
void Kethu9::updatePosition() {
    moveToTarget();
}

void Kethu9::setRandomTarget() {
...
void Kethu9::render(SDL_Renderer* renderer, SDL_Rect camera) {
    SDL_Texture* currentSprite = facingLeft ? spriteLeft : spriteRight;
    const int frameWidth = 2867 / 15;
...
float Kethu9::getX() const { return x; }
float Kethu9::getY() const { return y; }

SDL_Rect Kethu9::getCollisionBox() const {
    const int frameWidth = 2867 / 15;
    const int frameHeight = 656 / 5;
