...
void Kethu8::loadSpriteSheet(const std::string& leftPath, const std::string& rightPath, SDL_Renderer* renderer) {
...
void Kethu8::updatePosition() {
    moveToTarget();
}

void Kethu8::setRandomTarget() {
...
void Kethu8::render(SDL_Renderer* renderer, SDL_Rect camera) {
    SDL_Texture* currentSprite = facingLeft ? spriteLeft : spriteRight;
    const int frameWidth = 2815 / 14;
    const int frameHeight = 439 / 3;
    SDL_Rect srcRect;
...
float Kethu8::getX() const { return x; }
float Kethu8::getY() const { return y; }

SDL_Rect Kethu8::getCollisionBox() const {
    const int frameWidth = 2815 / 14;
    const int frameHeight = 439 / 3;
