...
void Kethu5::loadSpriteSheet(const std::string& leftPath, const std::string& rightPath, SDL_Renderer* renderer) {
...
void Kethu5::updatePosition() {
    moveToTarget();
}

void Kethu5::setRandomTarget() {
...
void Kethu5::render(SDL_Renderer* renderer, SDL_Rect camera) {
    SDL_Texture* currentSprite = facingLeft ? spriteLeft : spriteRight;
    const int frameWidth = 2535 / 14;
    const int frameHeight = 645 / 4;
    SDL_Rect srcRect;
...
float Kethu5::getX() const { return x; }
float Kethu5::getY() const { return y; }

SDL_Rect Kethu5::getCollisionBox() const {
    const int frameWidth = 2535 / 14;
    const int frameHeight = 645 / 4;
