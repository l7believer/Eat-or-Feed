...
void Kethu3::loadSpriteSheet(const std::string& leftPath, const std::string& rightPath, SDL_Renderer* renderer) {
...
void Kethu3::updatePosition() {
    moveToTarget();
}

void Kethu3::setRandomTarget() {
...
void Kethu3::render(SDL_Renderer* renderer, SDL_Rect camera) {
    SDL_Texture* currentSprite = facingLeft ? spriteLeft : spriteRight;
    const int frameWidth = 3795 / 14;
...
float Kethu3::getX() const { return x; }
float Kethu3::getY() const { return y; }

SDL_Rect Kethu3::getCollisionBox() const {
    const int frameWidth = 3795 / 14;
    const int frameHeight = 364 / 3;
