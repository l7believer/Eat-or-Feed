...
void Kethu6::loadSpriteSheet(const std::string& leftPath, const std::string& rightPath, SDL_Renderer* renderer) {
...
void Kethu6::updatePosition() {
    moveToTarget();
}

void Kethu6::setRandomTarget() {
...
void Kethu6::render(SDL_Renderer* renderer, SDL_Rect camera) {
    SDL_Texture* currentSprite = facingLeft ? spriteLeft : spriteRight;
    const int frameWidth = 3766 / 15;
    const int frameHeight = 529 / 3;
    SDL_Rect srcRect;
...
float Kethu6::getX() const { return x; }
float Kethu6::getY() const { return y; }

SDL_Rect Kethu6::getCollisionBox() const {
    const int frameWidth = 3766 / 15;
    const int frameHeight = 529 / 3;
