...
void Kethu2::loadSpriteSheet(const std::string& leftPath, const std::string& rightPath, SDL_Renderer* renderer) {
...
void Kethu2::updatePosition() {
    moveToTarget();
}

void Kethu2::setRandomTarget() {
...
void Kethu2::render(SDL_Renderer* renderer, SDL_Rect camera) {
    SDL_Texture* currentSprite = facingLeft ? spriteLeft : spriteRight;
    const int frameWidth = 3016 / 15;
    const int frameHeight = 725 / 4;
    SDL_Rect srcRect;
...
float Kethu2::getX() const { return x; }
float Kethu2::getY() const { return y; }

SDL_Rect Kethu2::getCollisionBox() const {
    const int frameWidth = 3016 / 15;
    const int frameHeight = 725 / 4;
